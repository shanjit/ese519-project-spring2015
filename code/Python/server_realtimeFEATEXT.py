...
###########   THE DATA FILE BEING STREAMED FROM THE Rpi.
    ...
    for line in f:
        j = 0
        Vref = 5;   #This value needs to be determined 5V right now. 
        tmp[:] = line.split(';');
        tmpInt = array([np.uint32(e) for e in tmp]);
        for foo in range(0,tmpInt.size):
            tmpInt[foo] = adc2float(tmpInt[foo], 24, Vref)
            
        tmpInt =  np.transpose(np.reshape(tmpInt, (128,8)));        
        data[:,(j*128):(j*128)+128] = tmpInt;
        j = j+1
